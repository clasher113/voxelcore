	...
	createPanel([this, path, fullName, actualName, docPath]() {
		...
		auto createStatesButton = [currentElement, panel, updatePreview](const std::wstring& buttonText, const std::string& attrName,
			std::vector<std::string>& statesList)
			{
				...
				auto button = std::make_shared<gui::Button>(buttonText + L": " + util::str2wstr_utf8(getAlign()), glm::vec4(10.f), gui::onaction());
		...
		auto createColorBox = [panel, currentElement, updatePreview](const std::string& labelText, const std::wstring& textboxPlaceholder,
			...
			{
				panel->add(std::make_shared<gui::Label>(labelText));
				auto color = std::make_shared<gui::TextBox>(textboxPlaceholder);
		...
		panel->add(std::make_shared<gui::Label>("Z-Index (depth)"));
		...
		if (currentTag & SLOTS_GRID) {
			...
			auto invEditorPanel = std::make_shared<gui::Panel>(panel->getSize());
		...
		if (currentTag & TEXTBOX) {
			panel->add(std::make_shared<gui::Label>("Validator function"));
		...
		panel->add(std::make_shared<gui::Button>(L"Save", glm::vec4(10.f), [this, xmlDoc, actualName](gui::GUI*) {
