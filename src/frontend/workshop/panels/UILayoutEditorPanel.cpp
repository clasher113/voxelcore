...
void workshop::WorkShopScreen::createUILayoutEditor(const fs::path& path, const std::string& fullName, std::vector<size_t> docPath) {
	createPanel([this, path, fullName, docPath]() {
		...
		auto getElement = [xmlDoc](const std::vector<size_t>& path) {
			xml::Node* result = const_cast<xml::Node*>(xmlDoc->getRoot());
			for (const size_t elem : path) {
				result = result->getElements()[elem].get();
			}
			return result;
		};
		...
		xml::Node* currentElement = getElement(docPath);
		xml::Node* previousElement = nullptr;
		...
		if (currentTag & INVENTORY || currentTag & CONTAINER || currentTag & PANEL) {
			panel << new gui::Button(L"Add element", glm::vec4(10.f),
				...
				createAddingUIElementPanel(panel.calcPos().x + panel.getSize().x,
					...
					auto node = std::make_unique<xml::Node>(name);
					...
					for (const auto& elem : uiElementsArgs.at(name).elemsTemplate) {
						auto tempNode = std::make_unique<xml::Node>(elem.first);
						tempNode->set(elem.second.first, elem.second.second);
						node->add(std::move(tempNode));
					}
					currentElement->add(std::move(node));
			...
			for (size_t i = 0; i < currentElement->getElements().size(); i++) {
				const xml::Node& elem = *currentElement->getElements()[i];
		...
		auto getElementText = [](xml::Node* element)->std::string {
			xml::Node* innerText = nullptr;
		...
		if (currentTag & HAS_ElEMENT_TEXT) {
			...
			textBox.setTextValidator([currentElement, &textBox, updatePreview](const std::wstring&) {
				...
				if (input.empty()) {
					...
					if (!innerText) {
						innerText = new xml::Node("#");
						currentElement->add(std::unique_ptr<xml::Node>(innerText));
					}
		...
		if (previousTag & PANEL || previousTag & CONTAINER || previousTag & INVENTORY) {
			auto moveElement = [previousElement, goTo, updatePreview, dp = docPath](int offset) mutable {
				std::vector<std::unique_ptr<xml::Node>>& elements = const_cast<std::vector<std::unique_ptr<xml::Node>>&>(previousElement->getElements());
