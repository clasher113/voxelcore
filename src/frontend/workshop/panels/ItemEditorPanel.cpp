...
void workshop::WorkShopScreen::createItemEditor(ItemDef& item) {
	createPanel([this, &item]() {
		...
		auto panel = std::make_shared<gui::Panel>(glm::vec2(settings.itemEditorWidth));
		if (!hasFile) {
			auto label = std::make_shared<gui::Label>(L"Autogenerated item from existing block\nCreate file to edit item propreties");
