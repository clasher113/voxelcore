...
#ifdef USE_DIRECTX
    const io::path shaderFile = paths.find(filename + ".hlsl");
    const std::string shaderSource = io::read_string(shaderFile);

    return [=](auto assets) mutable {
        assets->store(Shader::loadShader(shaderFile.string(), shaderSource), name);
    };
#elif USE_OPENGL
    io::path vertexFile = paths.find(filename + ".glslv");
    io::path fragmentFile = paths.find(filename + ".glslf");
...
#ifdef USE_DIRECTX
    const io::path effectShaderFile = paths.find(SHADERS_FOLDER + "/effect.hlsl");
    std::string effectSource = io::read_string(effectShaderFile);
    const std::string effectFile = paths.find(file + ".hlsl").string().substr(effectShaderFile.parent().string().size() + 1);

    const char* replaceStr = "__effect__";
    const size_t replacePos = effectSource.find(replaceStr);
    if (replacePos != std::string::npos) {
        effectSource.replace(replacePos, strlen(replaceStr), effectFile);
    }

    return [=](auto assets) mutable {
        std::unique_ptr<Shader> shader = Shader::loadShader(effectShaderFile.string(), effectSource);
        ...
        ID3DBlob* pixelByteCode = Shader::compileShader(effectSource, ShaderType::PIXEL);
        ConstantBufferBuilder::ParamsMap params = ConstantBufferBuilder::parseParams(pixelByteCode);
        pixelByteCode->Release();

        assets->store(std::make_shared<PostEffect>(advanced, std::move(shader), params), name);
    };
#elif USE_OPENGL
