	...
	m_description()
{
	...
	m_p_resourceView->GetDesc(&m_srvDescription);
...
void Texture::bind(ShaderType shaderType, UINT startSlot) const {
	ID3D11DeviceContext* const context = Device::getContext();
...
void Texture::unbind(ShaderType shaderType, UINT startSlot) const {
	ID3D11DeviceContext* const context = Device::getContext();
	ID3D11ShaderResourceView* nullSRV = { nullptr };
	if (shaderType & VERTEX)	context->VSSetShaderResources(startSlot, 1u, &nullSRV);
	if (shaderType & PIXEL)		context->PSSetShaderResources(startSlot, 1u, &nullSRV);
