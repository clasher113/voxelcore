...
#ifdef USE_DIRECTX
        auto data = std::make_unique<ubyte[]>(size.x * size.y * 4);
        auto texture = Device::getSurface();
...
#ifdef USE_DIRECTX
            Device::setSwapInterval(framerate == -1);
#elif USE_OPENGL
...
> Window::initialize(DisplaySettings* settings, std::string title) {
    ...
    Device::initialize(windowHandle, width, height);
    Device::setSwapInterval(1);

    const AdapterData& adapterData = Device::getAdapterData();
...
#ifdef USE_DIRECTX
    Device::setClearColor(color.r, color.g, color.b, 1.f);
#elif USE_OPENGL
